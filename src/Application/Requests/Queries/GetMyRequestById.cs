...
public class GetMyRequestByIdQueryHandler : IRequestHandler<GetMyRequestByIdQuery, Result<RequestDto>>
{
    private readonly IApplicationDbContext _context;
    private readonly IUser _user;
    private readonly ILocalizer _localizer;

    public GetMyRequestByIdQueryHandler(IApplicationDbContext context, IUser user, ILocalizer localizer)
    {
        _context = context;
        _user = user;
    ...
    public async Task<Result<RequestDto>> Handle(GetMyRequestByIdQuery query, CancellationToken cancellationToken)
    {
        var dto = await _context.Requests
            .AsNoTracking()
            .Select(x => x.MapToDto())
            .FirstOrDefaultAsync(x => x.Id == query.RequestId && x.CreatedBy == _user.Id, cancellationToken);
