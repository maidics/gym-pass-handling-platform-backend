...
internal record DeserializeRequestPayloadCommand<TPayload>(Request Request)
    : IRequest<Result<TPayload>>;
    ...
    : IRequestHandler<DeserializeRequestPayloadCommand<TPayload>, Result<TPayload>>
{
    ...
    public DeserializeRequestPayloadCommandHandler(
        ILogger<DeserializeRequestPayloadCommandHandler<TPayload>> logger
    )
    {
        _logger = logger;
    }

    public Task<Result<TPayload>> Handle(
        DeserializeRequestPayloadCommand<TPayload> command,
    ...
    )
    {
        if (command.Request.Payload is null)
        {
            ...
            return Task.FromResult(
                (Result<TPayload>)Result.InternalError("Request has no payload.")
            );
        }
        ...
        try
        {
            ...
            {
                return Task.FromResult(
                    (Result<TPayload>)Result.InternalError("Failed to deserialize request payload.")
                );
            }
        }
        catch
