...
[Authorize(Roles = $"{Roles.GymAdministrator},{Roles.GymStaff},{Roles.PendingGymEmployee},{Roles.User}")]
...
public class CancelMyRequestCommandValidator : AbstractValidator<CancelMyRequestCommand>
{
    ...
    {
        RuleFor(v => v.RequestId).NotEmpty()
            .WithMessage(
...
public class CancelMyRequestCommandHandler : IRequestHandler<CancelMyRequestCommand, Result>
{
    ...
    public CancelMyRequestCommandHandler(IApplicationDbContext context, IUser user, ILocalizer localizer)
    {
        _context = context;
        _user = user;
