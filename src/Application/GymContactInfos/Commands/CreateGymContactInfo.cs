...
public record CreateGymContactInfoCommand(
    string? PhoneNumber,
    string? Email,
    string FullName,
    Address? Address
    ...
    public CreateGymContactInfoDtoValidator(ILocalizer localizer)
    {
        ...
        RuleFor(v => v.FullName)
            .NotEmptyWithMaxLengthAndMessageLocalized(
                localizer,
                nameof(SharedResource.FullName),
                MaxLengths.FullName
            );
    }
}

public class CreateGymContactInfoCommandHandler
    : IRequestHandler<CreateGymContactInfoCommand, Result>
{
    ...
    public async Task<Result> Handle(
        CreateGymContactInfoCommand command,
        CancellationToken cancellationToken
    )
    {
        var gym = await _context
            .GymEmployments.Where(x => x.UserId == _user.Id)
            .Include(x => x.Gym)
            .Select(x => x.Gym)
