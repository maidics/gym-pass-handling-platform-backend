...
public class GymMembershipStatusChangedEventHandler
    : INotificationHandler<GymMembershipStatusChangedEvent>
{
    ...
    public async Task Handle(
        GymMembershipStatusChangedEvent notification,
    ...
    )
    {
        ...
        using (var scope = new CultureInfoScope(language))
        {
            model = new GymMembershipStatusChangedEmailModel
            {
                ...
                Subject = isBanned
                    ? _localizer.Get(
                        nameof(SharedResource.GymMembershipStatusBannedEmailSubject),
                        gymName
                    )
                    : _localizer.Get(
                        nameof(SharedResource.GymMembershipStatusUnbannedEmailSubject),
                        gymName
                    ),

                Greeting = _localizer.Get(nameof(SharedResource.EmailGreeting), result.FirstName),
                ...
                Body2 = isBanned
                    ? _localizer.Get(
                        nameof(SharedResource.GymMembershipStatusBannedEmailBody2),
                        gymName
                    )
                    : _localizer.Get(
                        nameof(SharedResource.GymMembershipStatusUnbannedEmailBody2),
                        gymName
                    ),
        ...
        var message =
            notification.NewStatus == GymMembershipStatus.Banned
                ? _localizer.GetForCulture(
                    language,
                    nameof(SharedResource.GymMembershipStatusBannedEmailSubject),
                    gymName
                )
