        ...
        where TEntity : class
    {
        ...
        if (entityType == null)
        {
            throw new InvalidOperationException(
                $"Entity type {typeof(TEntity).Name} not found in db context model."
            );
        }
        ...
        {
            throw new InvalidOperationException(
                $"Number of key values ({keyvalues.Length}) does not match the number of key properties ({keys?.Count ?? 0})."
            );
        }
